...
from transformers import Data2VecConfig, is_tf_available
...
if is_tf_available():
    ...
    from transformers import TFData2VecForCTC, TFData2VecModel, Wav2Vec2Processor
    from transformers.models.data2vec.modeling_tf_data2vec import _compute_mask_indices
...
class TFData2VecModelTester:
    ...
    def check_ctc_loss(self, config, input_values, *args):
        model = TFData2VecForCTC(config)

        input_values = input_values[:3]
        attention_mask = tf.ones_like(input_values)

        input_lengths = tf.constant([input_values.shape[-1] // i for i in [4, 2, 1]])
        max_length_labels = model.data2vec._get_feat_extract_output_lengths(input_lengths)
...
@require_tf
class TFData2VecModelTest(TFModelTesterMixin, unittest.TestCase):

    all_model_classes = (TFData2VecModel, TFData2VecForCTC) if is_tf_available() else ()
    test_resize_embeddings = False
    test_head_masking = False
    test_onnx = False

    def setUp(self):
        self.model_tester = TFData2VecModelTester(self)
...
@require_tf
class TFData2VecRobustModelTest(TFModelTesterMixin, unittest.TestCase):
    all_model_classes = (TFData2VecModel, TFData2VecForCTC) if is_tf_available() else ()
    test_resize_embeddings = False
    test_head_masking = False
    test_onnx = False
    ...
    def setUp(self):
        self.model_tester = TFData2VecModelTester(
            self,
            conv_stride=(3, 3, 3),
...
@require_tf
class TFData2VecUtilsTest(unittest.TestCase):
...
@require_tf
@slow
class TFData2VecModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_ctc_robust_batched(self):
        model = TFData2VecForCTC.from_pretrained("facebook/data2vec-large-960h-lv60-self")
    ...
    def test_data2vec_with_lm(self):
        ...
        model = TFData2VecForCTC.from_pretrained("patrickvonplaten/data2vec-large-xlsr-53-spanish-with-lm")
