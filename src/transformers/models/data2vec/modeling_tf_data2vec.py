...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2GroupNorm with Wav2Vec2->Data2Vec
class TFData2VecGroupNorm(tf.keras.layers.Layer):
    """
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2WeightNormConv1D with Wav2Vec2->Data2Vec
class TFData2VecWeightNormConv1D(tf.keras.layers.Conv1D):
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2NoLayerNormConvLayer with Wav2Vec2->Data2Vec
class TFData2VecNoLayerNormConvLayer(tf.keras.layers.Layer):
    def __init__(self, config: Data2VecConfig, layer_id: int = 0, **kwargs: Any) -> None:
        super().__init__(**kwargs)
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2LayerNormConvLayer with Wav2Vec2->Data2Vec
class TFData2VecLayerNormConvLayer(tf.keras.layers.Layer):
    def __init__(self, config: Data2VecConfig, layer_id: int = 0, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.in_conv_dim = config.conv_dim[layer_id] if layer_id > 0 else 1
        self.out_conv_dim = config.conv_dim[layer_id]
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2GroupNormConvLayer with Wav2Vec2->Data2Vec
class TFData2VecGroupNormConvLayer(tf.keras.layers.Layer):
    def __init__(self, config: Data2VecConfig, layer_id: int = 0, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.in_conv_dim = config.conv_dim[layer_id] if layer_id > 0 else 1
        self.out_conv_dim = config.conv_dim[layer_id]
        ...
        self.activation = get_tf_activation(config.feat_extract_activation)
        self.layer_norm = TFData2VecGroupNorm(
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2PositionalConvEmbedding with Wav2Vec2->Data2Vec
class TFData2VecPositionalConvEmbedding(tf.keras.layers.Layer):
    def __init__(self, config: Data2VecConfig, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.conv = TFData2VecWeightNormConv1D(
        ...
        self.padding = TFData2VecSamePadLayer(config.num_conv_pos_embeddings)
        self.activation = get_tf_activation(config.feat_extract_activation)
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2SamePadLayer with Wav2Vec2->Data2Vec
class TFData2VecSamePadLayer(tf.keras.layers.Layer):
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2FeatureEncoder with Wav2Vec2->Data2Vec
class TFData2VecFeatureEncoder(tf.keras.layers.Layer):
    def __init__(self, config: Data2VecConfig, **kwargs: Any) -> None:
        super().__init__(**kwargs)

        if config.feat_extract_norm == "group":
            conv_layers = [TFData2VecGroupNormConvLayer(config, layer_id=0, name=f"conv_layers.{0}")] + [
                TFData2VecNoLayerNormConvLayer(config, layer_id=i + 1, name=f"conv_layers.{i+1}")
                for i in range(config.num_feat_extract_layers - 1)
            ]
        elif config.feat_extract_norm == "layer":
            conv_layers = [
                TFData2VecLayerNormConvLayer(config, layer_id=i, name=f"conv_layers.{i}")
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2FeatureExtractor with Wav2Vec2->Data2Vec,Wav2Vec2FeatureExtractor->Wav2Vec2FeatureExtractor
class TFWav2Vec2FeatureExtractor(TFData2VecFeatureEncoder):
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2FeatureProjection with Wav2Vec2->Data2Vec
class TFData2VecFeatureProjection(tf.keras.layers.Layer):
    def __init__(self, config: Data2VecConfig, **kwargs):
        super().__init__(**kwargs)
...
# Copied from transformers.models.bart.modeling_tf_bart.TFBartAttention with TFBart->TFData2Vec
class TFData2VecAttention(tf.keras.layers.Layer):
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2FeedForward with Wav2Vec2->Data2Vec
class TFData2VecFeedForward(tf.keras.layers.Layer):
    def __init__(self, config: Data2VecConfig, **kwargs):
        super().__init__(**kwargs)
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2EncoderLayer with Wav2Vec2->Data2Vec
class TFData2VecEncoderLayer(tf.keras.layers.Layer):
    def __init__(self, config: Data2VecConfig, **kwargs):
        super().__init__(**kwargs)
        self.attention = TFData2VecAttention(
        ...
        self.layer_norm = tf.keras.layers.LayerNormalization(epsilon=config.layer_norm_eps, name="layer_norm")
        self.feed_forward = TFData2VecFeedForward(config, name="feed_forward")
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2EncoderLayerStableLayerNorm with Wav2Vec2->Data2Vec
class TFData2VecEncoderLayerStableLayerNorm(tf.keras.layers.Layer):
    def __init__(self, config: Data2VecConfig, **kwargs):
        super().__init__(**kwargs)
        self.attention = TFData2VecAttention(
            embed_dim=config.hidden_size,
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2Encoder with Wav2Vec2->Data2Vec
class TFData2VecEncoder(tf.keras.layers.Layer):
    def __init__(self, config: Data2VecConfig, **kwargs):
        super().__init__(**kwargs)
        self.config = config
        self.pos_conv_embed = TFData2VecPositionalConvEmbedding(config, name="pos_conv_embed")
        self.layer_norm = tf.keras.layers.LayerNormalization(epsilon=config.layer_norm_eps, name="layer_norm")
        self.dropout = tf.keras.layers.Dropout(config.hidden_dropout)
        self.layer = [TFData2VecEncoderLayer(config, name=f"layers.{i}") for i in range(config.num_hidden_layers)]
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2EncoderStableLayerNorm with Wav2Vec2->Data2Vec
class TFData2VecEncoderStableLayerNorm(tf.keras.layers.Layer):
    def __init__(self, config: Data2VecConfig, **kwargs):
        super().__init__(**kwargs)
        self.config = config
        self.pos_conv_embed = TFData2VecPositionalConvEmbedding(config, name="pos_conv_embed")
        ...
        self.dropout = tf.keras.layers.Dropout(config.hidden_dropout)
        self.layer = [
            TFData2VecEncoderLayerStableLayerNorm(config, name=f"layers.{i}") for i in range(config.num_hidden_layers)
        ]
...
@keras_serializable
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2MainLayer with Wav2Vec2->Data2Vec
class TFData2VecMainLayer(tf.keras.layers.Layer):
    config_class = Data2VecConfig

    def __init__(self, config: Data2VecConfig, **kwargs):
        super().__init__(**kwargs)
        self.config = config
        self.feature_extractor = TFData2VecFeatureEncoder(config, name="feature_extractor")
        self.feature_projection = TFData2VecFeatureProjection(config, name="feature_projection")

        if config.do_stable_layer_norm:
            self.encoder = TFData2VecEncoderStableLayerNorm(config, name="encoder")
        else:
            self.encoder = TFData2VecEncoder(config, name="encoder")
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2PreTrainedModel with Wav2Vec2->Data2Vec,wav2vec2->data2vec
class TFData2VecPreTrainedModel(TFPreTrainedModel):
    """
...
@add_start_docstrings(
    "The bare TFData2Vec Model transformer outputing raw hidden-states without any specific head on top.",
    DATA2VEC_START_DOCSTRING,
)
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2Model with Wav2Vec2->Data2Vec,wav2vec2->data2vec,WAV_2_VEC_2->DATA2VEC,facebook/wav2vec2-base-960h->data2vec
class TFData2VecModel(TFData2VecPreTrainedModel):
    def __init__(self, config: Data2VecConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.config = config
        self.data2vec = TFData2VecMainLayer(config, name="data2vec")
    ...
    def call(
        ...
        ```python
        >>> from transformers import Wav2Vec2Processor, TFData2VecModel
        >>> from datasets import load_dataset
        >>> import soundfile as sf

        >>> processor = Wav2Vec2Processor.from_pretrained("data2vec")
        >>> model = TFData2VecModel.from_pretrained("data2vec")
...
@add_start_docstrings(
    """TFData2Vec Model with a `language modeling` head on top for Connectionist Temporal Classification (CTC).""",
    DATA2VEC_START_DOCSTRING,
)
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2ForCTC with Wav2Vec2->Data2Vec,wav2vec2->data2vec,WAV_2_VEC_2->DATA2VEC,facebook/wav2vec2-base-960h->data2vec
class TFData2VecForCTC(TFData2VecPreTrainedModel):
    def __init__(self, config: Data2VecConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
    ...
    def freeze_feature_encoder(self):
        ...
        not be updated during training.
        """
        self.data2vec.feature_extractor.trainable = False
    ...
    def call(
        ...
        ```python
        >>> import tensorflow as tf
        >>> from transformers import Wav2Vec2Processor, TFData2VecForCTC
        >>> from datasets import load_dataset
        >>> import soundfile as sf

        >>> processor = Wav2Vec2Processor.from_pretrained("data2vec")
        >>> model = TFData2VecForCTC.from_pretrained("data2vec")
        ...
        if labels is not None:
            ...
            input_lengths = self.data2vec._get_feat_extract_output_lengths(tf.reduce_sum(attention_mask, axis=-1))
