...
def convert_classification(base_model_name, hf_config, downstream_dict):
    model = Data2VecForSequenceClassification.from_pretrained(base_model_name, config=hf_config)
...
def convert_diarization(base_model_name, hf_config, downstream_dict):
    model = Data2VecForAudioFrameClassification.from_pretrained(base_model_name, config=hf_config)
...
def convert_xvector(base_model_name, hf_config, downstream_dict):
    model = Data2VecForXVector.from_pretrained(base_model_name, config=hf_config)
...
def convert_s3prl_checkpoint(base_model_name, config_path, checkpoint_path, model_dump_path):
    ...
    hf_config = Data2VecConfig.from_pretrained(config_path)
